import json 
with open(r'C:\\Users\\098ku\\OneDrive\\Desktop\\assignment\New folder\\food_items.json') as food_list: # Enter the address of the json file created while running the application or creat one json file if not downloaded named (food_items.json)
    all_items=json.load(food_list)
...
def admin():
    ...
    input_name = input("Enter admin name:(admin)\n")
    input_password = input("Enter admin password:(admin)\n")
...
''')
            if option == "1":
                ...
                response2 = admin.edit_food_items(id1)
                if response2 == "sucess":
...
def register_user():
    ...
    user.set_password()
    try:
        if user.__dict__["email"] not in content.keys():
            content[user.__dict__["email"]] = user.__dict__
            file.seek(0)
            file.truncate()
        ...
        elif user.__dict__["email"] in content.keys():
            file.close()
            return "already"
    except KeyError:    
        content["email"] = user.__dict__
        file.seek(0)
        file.truncate()
        json.dump(content,file,indent=4)
        file.close()    
...
Enter 4 to exit   
    ''')
        ...
        if option_user == "1":
            ...
            reponse7 = user.palce_order(user_optn,user_mail)
            if reponse7 == "sucess":
...
''')
    ...
    if user_option == "1":
        while True:
            admin_input = input("please confirm again by entering yes for login /no for back to main first page.\n")
            if admin_input.lower()=="yes":    
                admin()
            elif admin_input.lower()=="no":
                print("Logged Out.\n")
                break
            else:
                print("Wrong option.\n")
        ...
        print("Choose from the above, The correct option.\n")
